...
public class UserController {
    ...
    // ADMIN — liste les comptes en attente
    @GetMapping("/pending")
    @PreAuthorize("hasRole('ADMIN')")
    ...
    // ADMIN — compter les comptes en attente ✅ CORRIGÉ (plus countByStatusPending)
    ...
    public ResponseEntity<Map<String, Object>> updateUser(
        ...
        if (previousRole != user.getRole()) {
            // handled by generic modification email below
            changed = true;
        }
    ...
    // ADMIN — bloquer un compte (empêche la connexion)
    @PutMapping("/{id}/lock")
    @PreAuthorize("hasRole('ADMIN')")
    ...
    @PreAuthorize("hasRole('ADMIN')")
        ...
        // Reset les tentatives échouées également
