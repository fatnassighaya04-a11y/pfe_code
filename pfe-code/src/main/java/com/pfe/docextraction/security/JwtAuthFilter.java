...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        // Si le token a un format de clé API (pas un JWT), on tente la clé API
    ...
    /** Une clé API ne contient pas de "." (alors qu'un JWT en a toujours 2). */
    ...
    private void authenticateWithApiKey(String key, HttpServletRequest request) {
        try {
            ...
            // Mettre à jour la date de dernière utilisation (détection externe)
            apiKey.setLastUsedAt(LocalDateTime.now());
            apiKeyRepository.save(apiKey);
        } catch (Exception ignored) {
            // clé invalide → pas d'authentification
        }
    }
