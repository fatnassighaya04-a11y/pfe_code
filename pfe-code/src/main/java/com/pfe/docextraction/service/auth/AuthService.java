...
public class AuthService {
    ...
    public AuthResponse register(RegisterRequest request) {
        ...
        User user = User.builder()
                ...
                .role(UserRole.OPERATEUR)   // à adapter si vous utilisez request.getRole()
    ...
    public AuthResponse login(AuthRequest request) {
        ...
        // 1. Statut du compte (workflow d'approbation)
        ...
        // 2. Compte bloqué par l'admin (ou auto-bloqué après tentatives échouées)
        ...
        // 3. Compte désactivé par l'admin
        ...
        try {
            ...
            // Mot de passe incorrect → incrémente le compteur d'échecs
            int attempts = user.getFailedAttempts() == null ? 0 : user.getFailedAttempts();
            attempts++;
            user.setFailedAttempts(attempts);

            // Auto-blocage après 5 tentatives échouées
            if (attempts >= 5) {
