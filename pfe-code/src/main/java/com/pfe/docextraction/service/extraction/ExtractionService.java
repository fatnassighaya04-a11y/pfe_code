...
public class ExtractionService {
    ...
    public void extractDocument(UUID documentId, double customThreshold) {
        ...
        try {
            ...
            String sourceText = extractSourceText(document);
            ...
            // Cherche le score à plusieurs endroits possibles (Gemini peut le mettre à la racine ou imbriqué)
            ...
            // Normalisation : si Gemini renvoie 0-1 au lieu de 0-100, multiplier
            if (confidenceValue > 0 && confidenceValue <= 1.0) {
                confidenceValue = confidenceValue * 100;
            }
            // Pas de score retourné → estimation basée sur le nombre de champs non-null
    ...
    /**
     * Si Gemini ne renvoie pas de confidence_score, on l'estime à partir
     * du nombre de champs non-null dans la réponse.
     */
    private double estimateConfidence(JsonNode extractedNode) {
        ...
        // Borne entre 50 et 95 pour rester réaliste
    ...
    private String extractSourceText(Document document) throws IOException {
        ...
        if (fileType == FileType.PDF) {
            // 1) Essai standard : extraction de la couche texte du PDF
            ...
            // 2) Fallback : PDF scanné → OCR Tesseract
            if (ocrService.isAvailable()) {
                System.out.println("📄 PDF sans couche texte détecté → OCR sur " + document.getOriginalFilename());
        ...
        if (isImage(fileType)) {
            // Pour les images, Gemini Vision lit directement.
            // Mais si OCR dispo, on peut lui donner aussi le texte OCR pour aider.
            if (ocrService.isAvailable()) {
