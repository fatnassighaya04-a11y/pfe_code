...
public class ApiKeyService {
    ...
    public void approveKey(UUID id) {
        ApiKey key = apiKeyRepository.findById(id).orElseThrow(() -> new RuntimeException("Clé non trouvée"));
        key.setIsActive(true);
        apiKeyRepository.save(key);
    }

    public ApiKey updateKey(UUID id, String appName, Integer expiresInDays) {
        ApiKey key = apiKeyRepository.findById(id).orElseThrow(() -> new RuntimeException("Clé non trouvée"));
        if (appName != null && !appName.isBlank()) key.setAppName(appName);
        if (expiresInDays != null) key.setExpiresAt(LocalDateTime.now().plusDays(expiresInDays));
